...
XENFT_CONTRACT_ADDRESS = os.environ['XENFT_CONTRACT_ADDRESS']
...
rpc_url = f"https://polygon-mainnet.infura.io/v3/{INFURA_ID}"  # Replace ABCDEF with your Infura API id
...
your_wallet_address = PUBLIC_KEY  # replace with the account from which we'll pay for XENFT (ensure you have sufficient funds)
...
your_wallet_address_private_key = os.environ['PRIVATE_KEY']
...
xenft_contract_address = web3.toChecksumAddress(XENFT_CONTRACT_ADDRESS)
xen_contract_address = web3.toChecksumAddress(XEN_CONTRACT_ADDRESS)
...
for i in range(1, num_of_xenft_to_mint + 1):
    ...
    matic_cost_in_usd = get_matic_usd_value()
    balance_before_claim = round(float(web3.fromWei(web3.eth.getBalance(your_wallet_address), 'ether')), 6)
    total_account_value = round(matic_cost_in_usd * balance_before_claim, 2)
    ...
    try:
        ...
        total_cost_in_usd = round(cost_in_ether * matic_cost_in_usd, 2)
        per_vmu_cost = round(total_cost_in_usd/vmu,2)
        print(f"{get_timestamp()} - Expected XENFT Cost: {cost_in_ether} MATIC (${total_cost_in_usd}) or ${per_vmu_cost}/VMU. Current MATIC value: ${matic_cost_in_usd}.")
        ...
        total_cost_in_usd = round((total_cost * matic_cost_in_usd), 2)
        per_vmu_cost = round(total_cost_in_usd / vmu, 2)
        total_account_value = round(matic_cost_in_usd * balance_after_claim, 2)
        print(f"{get_timestamp()} - Actual (final) XENFT Cost: {total_cost} MATIC (${total_cost_in_usd}) or ${per_vmu_cost}/VMU. Current MATIC value: ${matic_cost_in_usd}.")
